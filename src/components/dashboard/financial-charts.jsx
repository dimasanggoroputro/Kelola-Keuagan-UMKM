...
import { Coffee, ShoppingBag, CreditCard, Coins, Home, Receipt } from "lucide-react";
import { cn } from "@/lib/utils";
import { formatRupiah } from "@/lib/format";

const CATEGORY_MAP = {
  food:     { color: "stroke-emerald-500 dark:stroke-emerald-400",   bg: "bg-emerald-500",   label: "Kuliner",     Icon: Coffee },
  shopping: { color: "stroke-blue-500 dark:stroke-blue-400",         bg: "bg-blue-500",      label: "Belanja",     Icon: ShoppingBag },
  bills:    { color: "stroke-purple-500 dark:stroke-purple-400",       bg: "bg-purple-500",    label: "Tagihan",     Icon: CreditCard },
  salary:   { color: "stroke-amber-500 dark:stroke-amber-400",       bg: "bg-amber-500",     label: "Operasional", Icon: Coins },
  rent:     { color: "stroke-indigo-500 dark:stroke-indigo-400",       bg: "bg-indigo-500",    label: "Tempat",      Icon: Home },
  other:    { color: "stroke-zinc-400 dark:stroke-zinc-500",         bg: "bg-zinc-400",      label: "Lainnya",     Icon: Receipt },
};
    ...
    return (
      <div className="rounded-2xl border border-dashed border-stone-200 dark:border-zinc-800/60 bg-gradient-to-b from-white/60 to-stone-50/30 dark:from-zinc-900/20 dark:to-zinc-950/10 py-10 px-5 text-center">
        <div className="relative inline-flex mb-4">
          <div className="absolute -inset-2 rounded-full bg-purple-500/8 dark:bg-purple-500/5 blur-lg animate-pulse" style={{ animationDuration: '5s' }} />
        ...
        <h4 className="text-sm font-black text-zinc-800 dark:text-white tracking-tight">Belum Ada Pengeluaran</h4>
        <p className="mt-2 text-xs text-zinc-400 dark:text-zinc-500 font-medium leading-relaxed max-w-[240px] mx-auto">
          Catat pengeluaran toko untuk melihat grafik pembagian kategori otomatis.
        </p>
      </div>
    );
  }
        ...
        <div className="relative h-32 w-32 shrink-0">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none select-none text-center">
            {activeIndex !== null ? (
              ...
              <>
                <span className="text-[8px] font-extrabold text-zinc-400 dark:text-zinc-500 uppercase tracking-widest leading-none">
                  Total
                </span>
                <span className="text-xs font-black text-zinc-900 dark:text-white leading-none mt-1.5 truncate max-w-[80px]">
            ...
            return (
              ...
              >
                <div className="flex items-center gap-2 min-w-0">
                  <span className={cn("h-2.5 w-2.5 rounded-full shrink-0", slice.bg)} />
                ...
                <div className="flex items-center gap-1.5 shrink-0 ml-3">
                  ...
                  <span className="text-[9px] font-extrabold text-zinc-400 dark:text-zinc-500 uppercase min-w-[28px] text-right">
                    {slice.percentage}%
                  </span>
                </div>
              </div>
